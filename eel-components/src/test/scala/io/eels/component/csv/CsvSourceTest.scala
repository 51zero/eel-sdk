...
class CsvSourceTest extends WordSpec with Matchers {
  ...
  "CsvSource" should {
    ...
    "support null cell value option as null" in {
      ...
      val path = Paths.get(file)
      CsvSource(path).withNullValue(null).toFrame(1).toSet().map(_.values).toSet shouldBe
        Set(Vector("1", null, "3"))
    }
    "support null cell value replacement value" in {
      val file = getClass.getResource("/csvwithempty.csv").toURI()
      val path = Paths.get(file)
      CsvSource(path).withNullValue("foo").toFrame(1).toSet().map(_.values).toSet shouldBe
        Set(Vector("1", "foo", "3"))
    }
    "read from path" in {
      val file = getClass.getResource("/csvtest.csv").toURI()
