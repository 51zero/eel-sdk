...
class CsvSourceTest extends WordSpec with Matchers {
  ...
  "CsvSource" should {
    ...
    "support null cell value option as null" in {
      ...
      val path = Paths.get(file)
      CsvSource(path).withNullValue(null).toDataStream().toSet shouldBe
        Set(Vector("1", null, "3"))
    }
    "support null cell value replacement value" in {
      val file = getClass.getResource("/io/eels/component/csv/csvwithempty.csv").toURI()
      val path = Paths.get(file)
      CsvSource(path).withNullValue("foo").toDataStream().toSet shouldBe
        Set(Vector("1", "foo", "3"))
    }
    "read from path" in {
      val file = getClass.getResource("/io/eels/component/csv/csvtest.csv").toURI()
    ...
    "support reading header" in {
      val file = getClass.getResource("/io/eels/component/csv/csvtest.csv").toURI()
      val path = Paths.get(file)
      CsvSource(path).withHeader(Header.FirstRow).toDataStream().collect.toSet shouldBe
        Set(Vector("e", "f", "g"), Vector("1", "2", "3"), Vector("4", "5", "6"))
    }
    "support skipping header" in {
      val file = getClass.getResource("/io/eels/component/csv/csvtest.csv").toURI()
      val path = Paths.get(file)
      CsvSource(path).withHeader(Header.None).toDataStream().toSet shouldBe
    ...
    "support delimiters" in {
      ...
      val path = Paths.get(file)
      CsvSource(path).withDelimiter('|').toDataStream().collect.toSet shouldBe
        Set(Vector("e", "f", "g"))
      CsvSource(path).withDelimiter('|').withHeader(Header.None).toDataStream().toSet shouldBe
    ...
    "support skipping corrupt rows" ignore {
      ...
      val path = Paths.get(file)
      CsvSource(path).withHeader(Header.FirstRow).toDataStream().toVector shouldBe
        Vector(Vector("1", "2", "3"))
    }
  }
}
