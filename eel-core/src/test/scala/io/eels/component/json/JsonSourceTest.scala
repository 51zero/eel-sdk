...
class JsonSourceTest extends WordSpec with Matchers {
  ...
  "JsonSource" should {
    "read multiple json docs from a file" in {
      ...
      JsonSource(new Path(getClass.getResource("/io/eels/component/json/test.json").getFile)).toDataStream().toSet shouldBe
        Set(
    ...
    "support all primitives" in {
      ...
      JsonSource(new Path(getClass.getResource("/io/eels/component/json/prims.json").getFile)).toDataStream().toSet shouldBe
        Set(
    ...
    "support maps" in {
      ...
      JsonSource(new Path(getClass.getResource("/io/eels/component/json/maps.json").getFile)).toDataStream().toSet shouldBe
        Set(
    ...
    "support arrays" in {
      ...
      JsonSource(new Path(getClass.getResource("/io/eels/component/json/arrays.json").getFile)).toDataStream().toSet shouldBe
        Set(
