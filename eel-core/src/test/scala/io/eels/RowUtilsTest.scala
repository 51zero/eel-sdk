...
class RowUtilsTest extends WordSpec with Matchers {
  "RowUtils.rowAlign" should {
    "rowAlign should reorder in line with target schema" in {
      val row = Row(Schema(Field("a"), Field("b"), Field("c")), "aaaa", "bbb", "ccc")
      val targetSchema = Schema(Field("c"), Field("b"))
      RowUtils.rowAlign(row, targetSchema) shouldBe Row(Schema(Field("c"), Field("b")), "ccc", "bbb")
    }
    "rowAlign should lookup missing data" in {
      val row = Row(Schema(Field("a"), Field("b"), Field("c")), "aaaa", "bbb", "ccc")
      val targetSchema = Schema(Field("c"), Field("d"))
      RowUtils.rowAlign(row, targetSchema, Map("d" -> "ddd")) shouldBe Row(Schema(Field("c"), Field("d")), "ccc", "ddd")
    }
