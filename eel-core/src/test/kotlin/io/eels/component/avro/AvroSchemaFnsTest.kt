  ...
  init {
    ...
    "fromAvro" should {
      "convert avro unions [null, string] to nullable columns" with {
        ...
        avroSchemaToSchema(avro) shouldBe Schema(Column("str", ColumnType.String, true, Precision(0), Scale(0), true))
      }
      "convert avro unions [null, enum] to nullable columns" with {
        ...
        avroSchemaToSchema(avro) shouldBe Schema(Column("u", ColumnType.String, true, Precision(0), Scale(0), true))
      }
      "convert avro unions [null, double] to nullable double columns" with {
        ...
        val avro = SchemaBuilder.record("dummy").fields().name("u").`type`(union).noDefault().endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("u", ColumnType.Double, true, Precision(0), Scale(0), true))
      }
      "convert avro boolean fields" with {
        val avro = SchemaBuilder.record("dummy").fields().requiredBoolean("b").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("b", ColumnType.Boolean, false, Precision(0), Scale(0), true))
      }
      "convert avro float fields" with {
        val avro = SchemaBuilder.record("dummy").fields().requiredFloat("f").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("f", ColumnType.Float, false, Precision(0), Scale(0), true))
      }
      "convert avro double fields" with {
        val avro = SchemaBuilder.record("dummy").fields().requiredDouble("d").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("d", ColumnType.Double, false, Precision(0), Scale(0), true))
      }
      "convert avro fixed fields to string" with {
        ...
        avroSchemaToSchema(avro) shouldBe Schema(Column("f", ColumnType.String, false, Precision(0), Scale(0), true))
      }
      "convert avro enum fields to string" with {
        val enum = org.apache.avro.Schema.createEnum("myenum", null, null, Arrays.asList("a", "b"))
        val avro = SchemaBuilder.record("dummy").fields().name("e").`type`(enum).noDefault().endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("e", ColumnType.String, false, Precision(0), Scale(0), true))
      }
      "convert avro int fields" with {
        val avro = SchemaBuilder.record("dummy").fields().requiredInt("i").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("i", ColumnType.Int, false, Precision(0), Scale(0), true))
      }
      "convert avro long fields" with {
        val avro = SchemaBuilder.record("dummy").fields().requiredLong("l").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("l", ColumnType.Long, false, Precision(0), Scale(0), true))
      }
      "convert avro string fields" with {
        val avro = SchemaBuilder.record("dummy").fields().requiredString("s").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Column("s", ColumnType.String, false, Precision(0), Scale(0), true))
      }
    }
  }
}
