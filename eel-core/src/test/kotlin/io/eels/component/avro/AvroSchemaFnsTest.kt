  ...
  init {
    ...
    "fromAvro" should {
      "convert avro unions [null, string] to nullable columns" {
        ...
        avroSchemaToSchema(avro) shouldBe Schema(Field("str", FieldType.String, true, Precision(0), Scale(0), true))
      }
      "convert avro unions [null, enum] to nullable columns" {
        ...
        avroSchemaToSchema(avro) shouldBe Schema(Field("u", FieldType.String, true, Precision(0), Scale(0), true))
      }
      "convert avro unions [null, double] to nullable double columns" {
        ...
        val avro = SchemaBuilder.record("dummy").fields().name("u").`type`(union).noDefault().endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("u", FieldType.Double, true, Precision(0), Scale(0), true))
      }
      "convert avro boolean fields" {
        val avro = SchemaBuilder.record("dummy").fields().requiredBoolean("b").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("b", FieldType.Boolean, false, Precision(0), Scale(0), true))
      }
      "convert avro float fields" {
        val avro = SchemaBuilder.record("dummy").fields().requiredFloat("f").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("f", FieldType.Float, false, Precision(0), Scale(0), true))
      }
      "convert avro double fields" {
        val avro = SchemaBuilder.record("dummy").fields().requiredDouble("d").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("d", FieldType.Double, false, Precision(0), Scale(0), true))
      }
      "convert avro fixed fields to string" {
        ...
        avroSchemaToSchema(avro) shouldBe Schema(Field("f", FieldType.String, false, Precision(0), Scale(0), true))
      }
      "convert avro enum fields to string" {
        val enum = org.apache.avro.Schema.createEnum("myenum", null, null, Arrays.asList("a", "b"))
        val avro = SchemaBuilder.record("dummy").fields().name("e").`type`(enum).noDefault().endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("e", FieldType.String, false, Precision(0), Scale(0), true))
      }
      "convert avro int fields" {
        val avro = SchemaBuilder.record("dummy").fields().requiredInt("i").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("i", FieldType.Int, false, Precision(0), Scale(0), true))
      }
      "convert avro long fields" {
        val avro = SchemaBuilder.record("dummy").fields().requiredLong("l").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("l", FieldType.Long, false, Precision(0), Scale(0), true))
      }
      "convert avro string fields" {
        val avro = SchemaBuilder.record("dummy").fields().requiredString("s").endRecord()
        avroSchemaToSchema(avro) shouldBe Schema(Field("s", FieldType.String, false, Precision(0), Scale(0), true))
      }
    }
  }
}
