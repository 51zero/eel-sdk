...
/**
  ...
  * This means it will strip out of the row any fields/values which are not in the target schema.
  * If padNulls is true then for any fields in the target schema which are missing from the
  * input row, null will be used. If false, then an exception will be thrown.
  */
class RowNormalizerFn(targetSchema: StructType,
                      padNulls: Boolean) {
  ...
  private def isIncludedField(fieldName: String) = targetSchema.fieldNames.contains(fieldName)

  private def default(fieldName: String) = {
    if (padNulls) null
    else sys.error(s"Missing value in row for field=$fieldName; if you wish to use nulls for missing values then configure the sink")
  }

  def apply(row: Row): Row = {
    val map = row.schema.fieldNames().zip(row.values).toMap
    val values = targetSchema.fieldNames.filter(isIncludedField).map { it =>
